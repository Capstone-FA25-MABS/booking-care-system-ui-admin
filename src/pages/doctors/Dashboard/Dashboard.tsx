...
const doctorMetricOverrides: Partial<Record<AppointmentMetricKey, AppointmentMetricOverride>> = {
    total: {
        getSub: (stats) =>
...
const DoctorDashboard: React.FC = () => {
    ...
    // Doctor không cần system overview
    ...
    // Doctor không cần system overview như hospital
    ...
    const loadAiInsights = useCallback(async () => {
        ...
        try {
            ...
            const response = await AIService.generateInsightsForDoctor(doctorProfile.id, request);
    ...
    }, [aiDateRange.from, aiDateRange.to, doctorProfile?.id]);
    ...
    }, [activeDateRange.fromDate, activeDateRange.toDate, doctorProfile?.id]);
    ...
    const handleExportDashboard = useCallback(() => {
        try {
            ...
            const dashboardElement = document.getElementById('DoctorDashboardPage');
            ...
            const htmlContent = `
                ...
                <head>
                    <meta charset="UTF-8">
                    <title>Báo Cáo Dashboard - Bác Sĩ ${doctorProfile?.firstName} ${doctorProfile?.lastName}</title>
                    <style>
                ...
                <body>
                    <div class="print-header">
                        ...
                        <p class="subtitle">Bác Sĩ ${doctorProfile?.firstName} ${doctorProfile?.lastName}</p>
                        <p class="info">
                            📅 ${dateRangeText}<br>
    ...
    // Doctor không cần system overview metrics

    const overviewMetrics = useMemo(() => {
        if (!stats) return [];
        return buildAppointmentOverviewMetrics(stats, doctorMetricOverrides).map((metric) => {
        ...
        <div className={`content ${styles.dashboardPage}`} id="DoctorDashboardPage">
            <div className="d-flex align-items-sm-center flex-sm-row flex-column gap-2 mb-3 pb-3 border-bottom">
                <div className="flex-grow-1">
                    <h4 className="fw-bold mb-0">
                        Dashboard Bác sĩ {doctorProfile?.firstName} {doctorProfile?.lastName}
                    </h4>
                </div>
            ...
            <div className={styles.trendCard}>
                <div className={styles.cardHeader}>
                    <h5>AI phân tích hiệu suất bác sĩ, dự đoán tương lai</h5>
                </div>
