...
import { AppointmentCardData, getAppointmentTimeText } from '@/types/appointment.types';
...
const AppointmentCalendar: React.FC = () => {
    ...
    const fetchAppointments = useCallback(
                ...
                if (response.success && response.data?.appointments) {
                    const calendarEvents: CalendarEvent[] = response.data.appointments.map(
                        (apt: any) => {
                            ...
                            return {
                                id: apt.id,
                                title: `${apt.patientInfo?.firstName || ''} ${apt.patientInfo?.lastName || ''}`.trim(),
                                start: date,
                                end: endDate,
                                extendedProps: {
                                    appointmentData,
                                    patientAvatar: apt.patientInfo?.avatar,
                                },
        ...
        [doctorProfile?.id] // Don't add isLoading to dependency - it will cause infinite loop
    );

    // No useEffect needed - datesSet will be triggered automatically when calendar mounts
